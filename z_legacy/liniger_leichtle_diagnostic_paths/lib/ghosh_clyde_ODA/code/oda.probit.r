...
oda.probit <- function(xo,zo,niter,burnin,lam.spec=1)
  {
    library(MASS);
    xo <- as.matrix(xo); zo <- as.vector(zo);
    ...
    if(checkpd==0)
      {
       ...
       if (sum(gamma)>=1)
           {
                if (sum(gamma)==1)
      {Pbeta <- (lam[gamma==1]+xcxc[gamma==1,gamma==1])*phi;Vbeta <- 1/Pbeta} else
    {Pbeta <- phi*(diag(lam[gamma==1])+xcxc[gamma==1,gamma==1]); Vbeta <- solve(Pbeta);}
